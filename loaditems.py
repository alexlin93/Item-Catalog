...
item2 = Item(name="Chicken Burger", description="Juicy grilled chicken pat\
                 ty with tomato mayo and lettuce",
                 category=category1)
...
item7 = Item(name="Grilled Cheese Sandwich", description="On texas \
                 toast with American Cheese", category=category1)

session.add(item7)
session.commit()

item8 = Item(name="Veggie Burger", description="Made with freshest of \
                 ingredients and home grown spices", category=category1)
...
item1 = Item(name="Chicken Stir Fry", description="with your choice of \
                 noodles vegetables and sauces", category=category2)

session.add(item1)
session.commit()

item2 = Item(name="Peking Duck", description=" a famous duck dish from Beijing that has been prepared since the imperial era. The meat is prized for its thin, crisp skin, with authentic versions of the dish serving mostly the skin and little meat, sliced in front of the diners", category=category2)

session.add(item2)
session.commit()

item3 = Item(name="Spicy Tuna Roll", description="",
                 category=category2)

session.add(item3)
session.commit()

item4 = Item(name="Nepali Momo ", description="",
                 category=category2)

session.add(item4)
session.commit()

item5 = Item(name="Beef Noodle Soup", description="",
                 category=category2)
...
item3 = Item(name="Honey Boba Shaved Snow",
...
item4 = Item(name="Cauliflower Manchurian",
                 description="Golden fried cauliflower florets in a midly spiced soya,garlic sauce cooked with fresh cilantro, celery, chilies,ginger & green onion", category=category1)
...
item1 = Item(name="Shellfish Tower", description="",
                 category=category1)

session.add(item1)
session.commit()

item2 = Item(name="Chicken and Rice", description="",
                 category=category1)

session.add(item2)
session.commit()

item3 = Item(name="Mom's Spaghetti", description="",
                 category=category1)
...
item4 = Item(name="Nigiri SamplerMaguro, Sake, Hamachi, Unagi, Uni, TORO!",
                 description="", category=category1)
...
item3 = Item(name="Morels on toast (seasonal)",
                 description="Wild morel mushrooms fried in butter, served on herbed toast slice", category=category1)
...
item1 = Item(name="Super Burrito Al Pastor",
                 description="Marinated Pork, Rice, Beans, Avocado, Cilantro, Salsa, Tortille", category=category1)
